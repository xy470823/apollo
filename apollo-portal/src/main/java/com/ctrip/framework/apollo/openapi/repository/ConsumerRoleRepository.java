...
public interface ConsumerRoleRepository extends PagingAndSortingRepository<ConsumerRole, Long> {
  ...
  @Modifying
  @Query("UPDATE ConsumerRole SET IsDeleted = 1, DeletedAt = UNIX_TIMESTAMP_MILLIS(SYSTIMESTAMP), DataChange_LastModifiedBy = ?2 WHERE RoleId in ?1 and IsDeleted = false")
