...
public interface ReleaseRepository extends PagingAndSortingRepository<Release, Long> {
  ...
  @Modifying
  @Query("update Release set IsDeleted = 1, DeletedAt = UNIX_TIMESTAMP_MILLIS(SYSTIMESTAMP), DataChange_LastModifiedBy = ?4 where AppId=?1 and ClusterName=?2 and NamespaceName = ?3 and IsDeleted = false")
